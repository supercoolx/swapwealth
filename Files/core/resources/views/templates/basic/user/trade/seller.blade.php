<div class="alert alert-warning" role="alert">
    <p class="text-md">@lang('Sell') {{__($tradeRequestsDetails->crypto->name)}} @lang('using') {{__($tradeRequestsDetails->fiatGateway->name)}} @lang('with') {{__($tradeRequestsDetails->fiat->name)}} ({{__($tradeRequestsDetails->fiat->code)}}). {{__($general->sitename)}} @lang('user named') {{__($tradeRequestsDetails->buyer->fullname)}} @lang('wishes to buy') {{__($tradeRequestsDetails->crypto->name)}} @lang('from you')</p>

</div>

<p class="text-md mt-2">@lang('Once the buyer has confirmed your payment ') <span class="badge text-white badge--released">{{showAmount($tradeRequestsDetails->crypto_amount,8)}}</span> {{__($tradeRequestsDetails->advertisement->crypto->code)}}  @lang('will be available for release')</p>
...
<div class="accordion cmn-accordion accordion-arrow" id="accordionExample">
    <div class="card">
        ...
        <button class="btn btn-link btn-block text-left collapsed" type="button" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
            ...
            <p class="text-dark">@lang('Trade Information')</p>
        </button>
        </div>
...
@if ($tradeRequestsDetails->status == 2)
    ...
    @if ($lastTime > Carbon\Carbon::now())
        <div class="alert alert-warning" role="alert">
            <p class="text-md">@lang('Buyer can dispute this trade after') <span id="dispute-min">{{$remainingMin}}</span> @lang('minutes.')</p>
        </div>
        <hr>
    @else
        <div class="alert alert-warning" role="alert">
            <p class="text-md">@lang('Buyer can dispute this trade anytime. If you receive the amount please relese')</p>
        </div>
        <hr>
    @endif
    ...
    <div class="row mt-4">
        <div class="col-md-6">
            <button type="submit" class="cmn-btn bg--success w-100 user-action" data-value="1" data-route="{{route('user.trade.request.release')}}">@lang('Release') <i class="las la-check"></i></button>
        </div>
        <div class="col-md-6 mt-md-0 mt-3">
            <button type="submit" class="cmn-btn btn--danger w-100 user-action" data-value="3" data-route="{{route('user.trade.request.dispute')}}">@lang('Dispute') <i class="las la-times"></i></button>
        </div>
    </div>
    <hr>
@endif
    ...
    <script>
        (function($) {
            ...
            $('.user-action').on('click', function() {
                ...
                if (value == 1) {
                    actionMsz =`@lang('By proceeding with this action, this trade will be marked as released by you')`;
                } else if (value == 2) {
