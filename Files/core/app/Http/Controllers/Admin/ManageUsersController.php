...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $totalWithdraw = Withdrawal::where('user_id',$user->id)->where('status',1)->count();
        $totalTransaction = Transaction::where('user_id',$user->id)->count();
        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        $wallets = Wallet::where('user_id', $user->id)->with('crypto')->get();

        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','totalWithdraw','totalTransaction','countries','wallets'));
    }
    ...
    public function addSubBalance(Request $request, $id)
    {
        ...
        if ($request->act) {
            ...
            $notify[] = ['success', $wallet->crypto->symbol . $amount . ' has been subtracted from ' . $user->username . '\'s balance'];
        }
        return back()->withNotify($notify);
    }
    ...
    public function transactions(Request $request, $id)
    {
        ...
        if ($request->search) {
            ...
            $transactions = $user->transactions()->where('trx', $search)->with(['user','crypto'])->orderBy('id','desc')->paginate(getPaginate());
        ...
        $transactions = $user->transactions()->with(['user','crypto'])->orderBy('id','desc')->paginate(getPaginate());
        $emptyMessage = 'No transactions';
    ...
    public function deposits(Request $request, $id)
    {
        ...
        $deposits = $user->deposits()->orderBy('id','desc')->with(['crypto','user'])->paginate(getPaginate());
