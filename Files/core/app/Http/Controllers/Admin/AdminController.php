...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $widget['totalTrade'] = TradeRequest::count();
        $widget['totalCrypto'] = Crypto::count();
        $widget['totalFiat'] = Fiat::count();

        $withdrawals =  Crypto::withSum(['withdrawals'=>function($withdrawals){
            $withdrawals->where('status',1);
        }],'amount')->withSum(['withdrawals'=>function($withdrawals){
            $withdrawals->where('status',1);
        }],'charge')->latest()->get();

        $deposits =  Crypto::withSum(['deposits'=>function($withdrawals){
                    $withdrawals->where('status',1);
                }],'amount')->withSum(['deposits'=>function($withdrawals){
                    $withdrawals->where('status',1);
                }],'charge')->latest()->get();
        ...
        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart', 'withdrawals' , 'deposits'));
    }
